<template>
    <div>
        <VCard elevation="6">
            ...
            <VCardText>
                <VList>
                    <VListItem class="pa-0" v-for="(item, i) in birthdays">
                        <VListItemTitle class="text-body-2">{{ item.name }}</VListItemTitle>
                        <VListItemSubtitle class="text-caption">{{ item.position }}</VListItemSubtitle>
                        <template v-slot:prepend>
                            <VAvatar size="30" density="compact" class="pa-0"  rounded="0">
                                <VImg :src="item.image" :alt="item.name" />
                            </VAvatar>
                        </template>
                        <template v-slot:append>
                            <span class="text-caption"> {{ formatDate(item.birthdate) }}</span>
                        </template>
                    </VListItem>
                </VList>
            </VCardText>
